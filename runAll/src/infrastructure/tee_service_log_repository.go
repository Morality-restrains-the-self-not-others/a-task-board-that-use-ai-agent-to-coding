...
func (r *TeeServiceLogRepository) Clear(service string) {
	...
	if truncator, ok := r.file.(domain.ServiceLogFileTruncator); ok {
		if err := truncator.TruncateService(service); err != nil {
			log.Printf("[runAll] file log sink truncate failed service=%s: %v", service, err)
		}
	}
}
