...
func TestStatusStore_EnsureNamesDoesNotResetExisting(t *testing.T) {
	store := NewStatusStore()
	store.Init([]string{"a"})
	store.Update("a", StatusHealthy, "")

	store.EnsureNames([]string{"a", "b", " "})

	a := store.Get("a")
	if a == nil || a.Status != StatusHealthy {
		t.Fatalf("existing a = %+v, want healthy", a)
	}
	b := store.Get("b")
	if b == nil || b.Status != StatusPending {
		t.Fatalf("new b = %+v, want pending", b)
	}
}
...
func TestStatusStore_SetDependsOn(t *testing.T) {
	...
	store.Update("backend", StatusHealthy, "")
	store.SetDependsOn("frontend", []DepStatus{
		{Name: "backend", Status: StatusPending},
	})
...
func TestStatusStore_ErrorClearedOnStopped(t *testing.T) {
	store := NewStatusStore()
	store.Init([]string{"svc"})

	store.Update("svc", StatusFailed, "build failed: exit status 1")
	store.Update("svc", StatusStopped, "")
	...
	if got.Error != "" {
		t.Errorf("error should be cleared on stopped transition, got %q", got.Error)
	}
}
...
func TestStatusStore_RecordFailureIfStatus_DoesNotClobberRestarting(t *testing.T) {
	store := NewStatusStore()
	store.Init([]string{"svc"})
	store.Update("svc", StatusRetrying, "")
	if !store.CompareAndSwapStatus("svc", StatusRetrying, StatusRestarting) {
		t.Fatal("claim restart")
	}
	if store.RecordFailureIfStatus("svc", StatusRetrying, "readiness", "health_failed", "probe timeout") {
		t.Fatal("in-flight health must not record failure after restart claimed the service")
	}
	got := store.Get("svc")
	if got.Status != StatusRestarting {
		t.Fatalf("status = %q, want restarting", got.Status)
	}
	if got.FailureCode != "" {
		t.Fatalf("failure code leaked onto restarting service: %q", got.FailureCode)
	}
}

func TestStatusStore_CompareAndSwapUpdate_WritesError(t *testing.T) {
	store := NewStatusStore()
	store.Init([]string{"svc"})
	store.Update("svc", StatusRetrying, "")
	if !store.CompareAndSwapUpdate("svc", StatusRetrying, StatusFailed, "boom") {
		t.Fatal("expected CAS update")
	}
	got := store.Get("svc")
	if got.Status != StatusFailed || got.Error != "boom" {
		t.Fatalf("got status=%q error=%q", got.Status, got.Error)
	}
	if store.CompareAndSwapUpdate("svc", StatusRetrying, StatusFailed, "nope") {
		t.Fatal("CAS from stale retrying must fail")
	}
}
...
func TestStatusStore_SetDependsOnSyncsLiveDependencyStatus(t *testing.T) {
	store := NewStatusStore()
	store.Init([]string{"docker-redis", "ai-monitor"})
	store.SetDependsOn("ai-monitor", []DepStatus{{Name: "docker-redis", Status: StatusPending}})
	store.Update("docker-redis", StatusHealthy, "")

	got := store.Get("ai-monitor")
	if got.DependsOn[0].Status != StatusHealthy {
		t.Fatalf("after Update: dep status = %q, want healthy", got.DependsOn[0].Status)
	}

	// Config reload / precise-restart re-applies depends_on as pending even though
	// the dependency is already healthy. Indicators must keep the live status.
	store.SetDependsOn("ai-monitor", []DepStatus{{Name: "docker-redis", Status: StatusPending}})
	got = store.Get("ai-monitor")
	if got.DependsOn[0].Status != StatusHealthy {
		t.Fatalf("after SetDependsOn pending: dep status = %q, want healthy (live overlay)", got.DependsOn[0].Status)
	}
}

func TestStatusStore_CompareAndSwapStatusSyncsDependencyDisplay(t *testing.T) {
	store := NewStatusStore()
	store.Init([]string{"upstream", "downstream"})
	store.SetDependsOn("downstream", []DepStatus{{Name: "upstream", Status: StatusPending}})
	store.Update("upstream", StatusHealthy, "")
	if got := store.Get("downstream").DependsOn[0].Status; got != StatusHealthy {
		t.Fatalf("setup dep status = %q, want healthy", got)
	}

	if !store.CompareAndSwapStatus("upstream", StatusHealthy, StatusRestarting) {
		t.Fatal("expected swap to succeed")
	}
	if got := store.Get("downstream").DependsOn[0].Status; got != StatusRestarting {
		t.Fatalf("dep status after CAS = %q, want restarting", got)
	}
}

func TestStatusStore_SetReadinessUpdatesDependencyDisplay(t *testing.T) {
	store := NewStatusStore()
	store.Init([]string{"saas-backend", "taskFE"})
	store.SetDependsOn("taskFE", []DepStatus{{Name: "saas-backend", Status: StatusPending}})
