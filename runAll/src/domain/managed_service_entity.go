...
// IsCanaryRestartableStatus reports whether rolling canary restart should include
// the service: idle (would be started) or already serving (would be overlapped).
func IsCanaryRestartableStatus(status string) bool {
	if IsStartableServiceStatus(status) {
		return true
	}
	switch status {
	case ServiceStatusHealthy, ServiceStatusRetrying:
		return true
	default:
		return false
	}
