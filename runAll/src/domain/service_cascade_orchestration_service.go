...
func (s *ServiceCascadeOrchestrationService) PlanStartAll() (ServiceLifecyclePlan, error) {
	nodes, index, err := s.loadTopologyIndex()
	if err != nil {
		return ServiceLifecyclePlan{}, err
	}
	if len(nodes) == 0 {
		return NewServiceLifecyclePlan(LifecycleOperationStart, nil)
	}

	closure := make(map[string]struct{}, len(nodes))
	for _, node := range nodes {
		if node.SkipStartAll {
			continue
		}
		closure[node.Name] = struct{}{}
	}
...
// PlanCanaryRestartAll returns start-all topology order including currently
// healthy services so rolling overlap can bounce the running stack without StopAll.
func (s *ServiceCascadeOrchestrationService) PlanCanaryRestartAll() (ServiceLifecyclePlan, error) {
	nodes, index, err := s.loadTopologyIndex()
	if err != nil {
		return ServiceLifecyclePlan{}, err
	}
	if len(nodes) == 0 {
		return NewServiceLifecyclePlan(LifecycleOperationCanaryRestart, nil)
	}
	...
	filtered, err := s.filterCanaryRestartable(order)
	if err != nil {
		return ServiceLifecyclePlan{}, err
	}
	return NewServiceLifecyclePlan(LifecycleOperationCanaryRestart, filtered)
}

func (s *ServiceCascadeOrchestrationService) PlanStopAll() (ServiceLifecyclePlan, error) {
	nodes, index, err := s.loadTopologyIndex()
	if err != nil {
		return ServiceLifecyclePlan{}, err
	}
	if len(nodes) == 0 {
		return NewServiceLifecyclePlan(LifecycleOperationStop, nil)
	}
	...
	stopOrder := reverseNames(order)
	filtered, err := s.filterStoppableAll(stopOrder)
	if err != nil {
		return ServiceLifecyclePlan{}, err
	}
	return NewServiceLifecyclePlan(LifecycleOperationStop, filtered)
...
func (s *ServiceCascadeOrchestrationService) filterCanaryRestartable(order []string) ([]string, error) {
	filtered := make([]string, 0, len(order))
	for _, name := range order {
		managed, err := s.runtime.FindByName(name)
		if err != nil {
		...
		if IsCanaryRestartableStatus(managed.Status) {
			filtered = append(filtered, name)
		}
	}
	return filtered, nil
...
// filterStoppableAll filters services for a "stop all" operation.
// Any service that is not already stopped/pending/skipped is a candidate.
func (s *ServiceCascadeOrchestrationService) filterStoppableAll(order []string) ([]string, error) {
	filtered := make([]string, 0, len(order))
	for _, name := range order {
		managed, err := s.runtime.FindByName(name)
		if err != nil {
