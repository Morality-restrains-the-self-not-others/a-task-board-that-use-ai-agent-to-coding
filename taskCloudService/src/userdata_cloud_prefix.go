...
func buildUserdataTaskCloudPrefix(taskAPIEndpoint, tenantID, workspaceID, taskID, commentID string) string {
...
func applyUserdataCloudPrefixRewrite(text string, p userdataReplaceParams) string {
	cp := buildUserdataTaskCloudPrefix(p.taskAPIEndpoint, p.tenantID, p.workspaceID, p.taskID, p.commentID)
	bare := strings.TrimRight(strings.TrimSpace(p.taskAPIEndpoint), "/")
	if text == "" || cp == "" || bare == "" {
		return text
	}
	out := rewriteUserdataBareAPIOriginTaskExports(text, cp, userdataBareOriginStrings(bare))
	return rewriteUserdataBrowserTaskDetailURL(out, cp, p.tenantID, p.workspaceID, p.taskID)
}

func userdataBareOriginStrings(bare string) []string {
	variants := userdataAlternateSchemeOrigins(bare)
...
func userdataAlternateSchemeOrigins(origin string) []string {
...
func rewriteUserdataBareAPIOriginTaskExports(text, cloudPrefix string, bareOrigins []string) string {
	if text == "" || cloudPrefix == "" {
		return text
	}
	out := text
...
func rewriteUserdataBrowserTaskDetailURL(text, cloudPrefix, tenantID, workspaceID, taskID string) string {
	tid := strings.TrimSpace(tenantID)
	wid := strings.TrimSpace(workspaceID)
	tk := strings.TrimSpace(taskID)
...
func rewriteUserdataLocalhostVerifyHosts(plain, publicBase string) string {
