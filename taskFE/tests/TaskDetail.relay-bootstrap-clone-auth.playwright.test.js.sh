...
export PLAYWRIGHT_RELAY_TASK_ID="${PLAYWRIGHT_RELAY_TASK_ID:-task_12590983282794675865}"
export PLAYWRIGHT_TENANT_ID="${PLAYWRIGHT_TENANT_ID:-850256677331562496}"
export CDP_URL="${PW_CDP_URL:-http://127.0.0.1:9222}"

# 优先 CDP 直连脚本（与仓库既有 relay-*-cdp.mjs 一致）；可用 RUN_PW_TEST=1 跑正式 test runner。
if [[ "${RUN_PW_TEST:-0}" == "1" ]]; then
  ...
  exec npx playwright test -c "$CONFIG" \
    "tests/TaskDetail.relay-bootstrap-clone-auth.playwright.test.js" \
    "$@"
fi

exec node tests/TaskDetail.relay-bootstrap-clone-auth-cdp.mjs "$@"
