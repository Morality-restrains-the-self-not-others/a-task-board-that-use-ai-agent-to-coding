...
// 定义组件事件
const emit = defineEmits(['workspace-created', 'cancel'])
...
const handleSubmit = async () => {
  ...
  await submitGuard.run(async ({ idempotencyKey }) => {
    ...
    try {
      ...
      // 发送创建工作空间请求
      const response = await apiFetch(`/api/projects/workspaces/tenant_id/${props.tenantId}`, {
        method: 'POST',
      ...
      if (response.ok) {
        ...
        // 发射工作空间创建成功事件
        emit('workspace-created', workspaceData)
      } else {
        // 处理错误响应
        ...
        if (!Object.keys(errors.value).length) {
          showRequestError('创建失败：' + errorMessage, { ...errorData, traceId: errorData.trace_id || errorData.traceId })
        }
      }
    } catch (error) {
