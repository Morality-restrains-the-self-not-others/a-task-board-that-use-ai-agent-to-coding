...
if (!process.env.VITEST) {
  console.log('[skip] SystemAdminUsers.tenant-company-column.test.js requires vitest runtime')
} else {
  ...
  describe('SystemAdminUsers tenant company column', () => {
    beforeEach(() => {
      apiFetch.mockReset()
    ...
    it('places 所属租户公司 after 邮箱 in the table header', async () => {
      ...
      const companyIdx = headers.indexOf('所属租户公司')
      expect(emailIdx).toBeGreaterThanOrEqual(0)
      expect(companyIdx).toBe(emailIdx + 1)
    })
  })
}
