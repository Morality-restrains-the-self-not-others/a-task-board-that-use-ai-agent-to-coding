...
if (!process.env.VITEST) {
  ...
  vi.mock('vue-router', () => ({
    ...
    useRoute: () => ({ params: { tenant: 'ten1' } }),
  }))
  // OPT-20260819-038: 让 guard 直接放行并把幂等键透传给 mergeIdempotencyHeaders，
  // 断言 POST 携带 Idempotency-Key 头。
  vi.mock('../utils/clickGuard.js', () => ({
    createClickGuard: () => ({
      run: async (fn) => fn({ idempotencyKey: 'ik-test-create-ws-view' }),
      isBusy: () => false,
    }),
    mergeIdempotencyHeaders: (headers, key) => ({ ...(headers || {}), 'Idempotency-Key': key }),
  }))

  const { default: View } = await import('./CreateWorkspace.vue')

  describe('CreateWorkspace(视图) 写操作 clickGuard 接线', () => {
    let apiFetch
    beforeEach(() => {
    ...
    it('创建工作空间 POST 携带 Idempotency-Key', async () => {
      ...
      expect(postCall[1].headers['Idempotency-Key']).toBe('ik-test-create-ws-view')
