...
func runDataMigrateFromDir(dsn string, repoRoot string) error {
	migDir := filepath.Join(repoRoot, "dataMigrate", "taskBill")
	...
	// Backfill from legacy taskbill_schema_migrations into data_migrate_log
	var count int
	if err := conn.QueryRow(`SELECT COUNT(*) FROM data_migrate_log`).Scan(&count); err == nil && count == 0 {
		rows, err := conn.Query(`SELECT name, applied_at FROM taskbill_schema_migrations ORDER BY name`)
		if err == nil {
			defer rows.Close()
			for rows.Next() {
