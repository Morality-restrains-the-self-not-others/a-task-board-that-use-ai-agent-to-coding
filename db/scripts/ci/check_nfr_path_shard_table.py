#!/usr/bin/env python3
"""NFR 文档「路径分片键审视」表门禁（OPT-20260812-030）。

元规则 43 / 5-nfr Hard Gate 要求 NFR 澄清文档含 `## 路径分片键审视` 表；本脚本
...
用法:
  check_nfr_path_shard_table.py                 # warn 全量
  check_nfr_path_shard_table.py --strict --files 'docs/superpowers/plans/2026-08-*.md'
"""
...
SECTION_RE = re.compile(r"^##\s*路径分片键审视\s*$", re.M)


def scan() -> list[Path]:
    """返回缺「路径分片键审视」节的 NFR 澄清文档。"""
    missing: list[Path] = []
    if not PLANS.is_dir():
        return missing
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.files:
        ...
        for pattern in args.files.split(","):
            ...
            # 兼容相对 meta 根的路径（glob 已相对 ROOT 匹配）
            targets.extend(hits)
        # 去重保序
        seen: set[Path] = set()
    ...
    for p in missing:
        ...
        if args.strict:
            print(f"ERROR {rel}: 缺少「## 路径分片键审视」表（硬约束 43 / 5-nfr）")
        else:
            print(f"WARN  {rel}: 缺少「## 路径分片键审视」表（存量容忍，新文档应补）")
    if missing:
        print(f"nfr-path-shard-table: {len(missing)} file(s) missing section")
    else:
        print(f"ok: {len(files)} NFR file(s) contain 路径分片键审视")
