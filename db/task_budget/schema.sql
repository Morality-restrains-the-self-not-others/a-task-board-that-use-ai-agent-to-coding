-- LLM Budget ledger schema (task_budget MySQL)
-- Owned by taskCloudService.
-- ⚠️  REFERENCE COPY — 权威 SSOT: dataMigrate/taskBudget/001_schema.sql
-- 该文件由 db/task_budget/migrate_from_saas.sh（SaaS SQLite→SQLite 一次性迁移）引用。
-- 所有 DDL 变更必须在 dataMigrate/taskBudget/ 中完成，然后同步到此副本。
-- Note: utf8mb4 unique key budget is 3072 bytes — keep composite keys ≤ ~700 chars.
