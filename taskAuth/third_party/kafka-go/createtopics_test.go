...
func TestClientCreateTopics(t *testing.T) {
	// Use unique topic names: fixed names would collide with topics left
	// behind by a previous run that failed before the deferred cleanup,
	// making the next run fail with TopicAlreadyExists.
	topic1 := makeTopic()
	topic2 := makeTopic()
	topic3 := makeTopic()
	...
	res, err := client.CreateTopics(context.Background(), &CreateTopicsRequest{
		Topics: []TopicConfig{
			{
				// The replica assignment references broker IDs explicitly;
				// the broker ID depends on the environment (the repo's
				// docker-compose broker is ID 1), so let the cluster place
				// the partitions instead.
				Topic:             topic1,
				NumPartitions:     3,
				ReplicationFactor: 1,
