...
func TestConn(t *testing.T) {
	...
	t.Run("nettest", func(t *testing.T) {
		...
		if os.Getenv("KAFKA_VERSION") == "" {
			// The nettest battery is timing-sensitive and known to flake
			// against brokers that are not the pinned test broker (see the
			// comment above; upstream CI pins a broker version via
			// KAFKA_VERSION). Environments that do not pin a broker version
			// get the same behavior as upstream CI, minus the flake.
			t.Log("skipping nettest because KAFKA_VERSION is not set")
			t.Skip()
		}
...
// isLoopbackHost reports whether host is a loopback address. The broker host
// advertised by the cluster depends on the environment the tests run in (the
// repo's docker-compose broker advertises "localhost", other environments may
// advertise a loopback IP), so tests assert on the host being loopback rather
// than on a specific value.
...
func testController(t *testing.T, conn *Conn) {
	...
	if !isLoopbackHost(b.Host) {
		t.Errorf("expected loopback host received %s", b.Host)
	}
	if b.Port != 9092 {
		t.Errorf("expected 9092 received %d", b.Port)
	}
	if b.ID <= 0 {
		t.Errorf("invalid broker id: %d", b.ID)
	}
	if b.Rack != "" {
...
func testBrokers(t *testing.T, conn *Conn) {
	...
	if !isLoopbackHost(brokers[0].Host) {
		t.Errorf("expected loopback host received %s", brokers[0].Host)
	}
	if brokers[0].ID <= 0 {
		t.Errorf("invalid broker id: %d", brokers[0].ID)
	}
}
...
func TestUnsupportedSASLMechanism(t *testing.T) {
	...
	if err := conn.saslHandshake("FOO"); !errors.Is(err, UnsupportedSASLMechanism) {
		if errors.Is(err, IllegalSASLState) {
			// The test environment's broker does not expose a SASL listener
			// on 9093 (the repo's docker-compose broker does, with PLAIN and
			// SCRAM mechanisms enabled).
			t.Skip("broker does not expose a SASL listener on 127.0.0.1:9093")
		}
