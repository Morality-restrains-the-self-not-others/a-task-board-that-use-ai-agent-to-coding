...
func TestConsumerGroupWithGroupTopicsMultple(t *testing.T) {
	// The broker auto-creates topics on demand, but a write racing the
	// topic's leader election fails with UnknownTopicOrPartition and leaves
	// the group without partitions to consume; create the topics up front so
	// the group subscription is stable. Creating topics can take a few
	// seconds on a loaded broker, so it must not consume the read budget.
	topics := []string{makeTopic(), makeTopic()}
	for _, topic := range topics {
		createTopic(t, topic, 1)
	}
	defer deleteTopic(t, topics...)
...
// closedAddr returns a loopback address with no listener, for tests that
// exercise "cannot connect" paths. The docker-compose broker used to run a
// SASL-only listener on localhost:9093, which made it a good target for
// these tests, but other environments may expose a plaintext listener there.
