...
func TestWechatPayCipher_EncryptWithoutSerial(t *testing.T) {
	...
	// getEncryptSerial 已被编译器内联，gomonkey 补丁无法拦截，
	// 故改为直接调用 cipherStringField 覆盖「ctx 中缺失加密序列号」的防御性错误分支。
	field, ok := reflect.TypeOf(s).FieldByName("Name")
	require.True(t, ok)
	fieldValue := reflect.ValueOf(&s).Elem().FieldByName("Name")

	err := c.cipherStringField(context.Background(), cipherTypeEncrypt, field, fieldValue)
	assert.Error(t, err)
}
