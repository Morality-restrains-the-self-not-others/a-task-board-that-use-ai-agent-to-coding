  ...
  test('runLayerOauthRefreshPush 把 GitLab match-key token 传给 oauthAuthByRepo', async () => {
    ...
    assert.equal(
      spawnSync('git', [
        ...
        'add',
        'origin',
        'https://gitlab-tencent-sh-1.daydaymoney.com/example-user/somanyad.git',
    ...
    const server = http.createServer((req, res) => {
      if (req.method === 'POST' && req.url?.includes('layer-github-oauth-access-tokens')) {
        ...
        res.end(
          JSON.stringify({
            ok: true,
            github_auth_by_repo: { 'example-user/somanyad': 'glpat-slug' },
            git_auth_by_repo_match_key: {
              'gitlab-tencent-sh-1.daydaymoney.com/example-user/somanyad': 'glpat-match',
            },
    ...
    try {
      ...
      assert.equal(httpStatus, 200);
      assert.equal(pushOpts?.oauthAuthByRepo?.['https://gitlab-tencent-sh-1.daydaymoney.com/example-user/somanyad']?.provider, 'gitlab');
      assert.equal(
        pushOpts?.oauthAuthByRepo?.['https://gitlab-tencent-sh-1.daydaymoney.com/example-user/somanyad']?.access_token,
        'glpat-match',
      );
      assert.equal(pushOpts?.accessTokenByRepoSlug?.['example-user/somanyad'], 'glpat-slug');
