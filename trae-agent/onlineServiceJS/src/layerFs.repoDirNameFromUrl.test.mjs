...
describe('repoDirNameFromUrl', () => {
  ...
  it('parses scp-style urls', () => {
    assert.equal(
      repoDirNameFromUrl('git@gitlab.daydaymoney.com:example-user/somanyad.git'),
      'somanyad',
    );
    assert.equal(
      repoDirNameFromUrl('git@gitlab.daydaymoney.com:example-user/somanyad-emailD.git'),
      'somanyad-emailD',
    );
  });

  it('keeps distinct names for sibling repos', () => {
    const a = repoDirNameFromUrl('git@gitlab.daydaymoney.com:example-user/somanyad.git');
    const b = repoDirNameFromUrl('git@gitlab.daydaymoney.com:example-user/somanyad-emailD.git');
    assert.notEqual(a, b);
  });
});
