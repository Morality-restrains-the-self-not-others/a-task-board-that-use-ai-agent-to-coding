...
test('buildTaskCloudPrefix: 必须含 comment，禁止旧 /cloud', () => {
  assert.equal(
    buildTaskCloudPrefix('https://api.daydaymoney.com', 't1', 'w1', 'task1', 'cmt_1'),
    'https://api.daydaymoney.com/api/tenant/t1/workspace/w1/task/task1/comment/cmt_1/cloud',
  );
  assert.equal(buildTaskCloudPrefix('https://api.daydaymoney.com', 't1', 'w1', 'task1', ''), '');
  assert.equal(buildTaskCloudPrefix('https://api.daydaymoney.com', 't1', 'w1', 'task1', '-'), '');
});
