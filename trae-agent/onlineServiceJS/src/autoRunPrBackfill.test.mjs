  ...
  const urls = extractPrUrlsFromPushResult({
    payload: {
      ...
      github_oauth_multirepo: {
        repos: [
          { pr: { html_url: 'https://gitlab-tencent-sh-1.daydaymoney.com/ljy/somanyad/-/merge_requests/2' } },
        ],
      },
    },
  });
  assert.deepEqual(urls, [
    'https://gitlab-tencent-sh-1.daydaymoney.com/ljy/somanyad/-/merge_requests/2',
  ]);
});
