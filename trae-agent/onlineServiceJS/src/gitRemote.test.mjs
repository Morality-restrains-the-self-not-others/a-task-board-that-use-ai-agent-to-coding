...
test('parseGitSshRepoUrl: SCP 风格', () => {
  assert.deepEqual(parseGitSshRepoUrl('git@gitlab.daydaymoney.com:example-user/somanyad.git'), {
    host: 'gitlab.daydaymoney.com',
    path: 'example-user/somanyad',
  });
});
...
test('normalizeRepoUrlForHttpsClone: 优先 https_clone_url', () => {
  assert.equal(
    normalizeRepoUrlForHttpsClone('git@gitlab.daydaymoney.com:example-user/somanyad.git', {
      httpsCloneUrl: 'http://127.0.0.1:8012/example-user/somanyad',
    }),
    'http://127.0.0.1:8012/example-user/somanyad',
  );
});
...
test('normalizeRepoUrlForHttpsClone: 自托管无 website 时保留 SSH（避免错端口）', () => {
  assert.equal(
    normalizeRepoUrlForHttpsClone('git@gitlab.daydaymoney.com:example-user/somanyad.git'),
    'git@gitlab.daydaymoney.com:example-user/somanyad.git',
  );
});
