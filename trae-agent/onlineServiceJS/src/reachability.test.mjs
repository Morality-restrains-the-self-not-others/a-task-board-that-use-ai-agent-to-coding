...
test('reachabilityFromBusinessEndpointEnv：域名仍写 :8765 且 TRAE_HOST_HTTP_PORT 为映射口时改用映射口', () => {
  ...
  try {
    ...
    process.env.BUSINESS_API_ENDPOINT = 'http://debug.daydaymoney.com:8765/api';
    delete process.env.BusinessApiEndPoint;
    const got = reachabilityFromBusinessEndpointEnv();
    assert.deepStrictEqual(got, {
      businessApiEndpoint: 'http://debug.daydaymoney.com:49152/api',
      serverUrl: 'http://debug.daydaymoney.com:49152',
      publicIp: null,
    });
  } finally {
    restoreEnv(saved);
