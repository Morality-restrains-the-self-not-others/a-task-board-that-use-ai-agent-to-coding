...
// Handler records TASK_REVISION_RECORDED as structured observability logs.
type Handler struct{}
...
func (h *Handler) Dispatch(ctx context.Context, cmd domain.DomainCommand) (domain.DispatchOutcome, error) {
	_ = ctx
	if cmd.EventType != "TASK_REVISION_RECORDED" {
	...
	if len(cmd.Envelope.Data) > 0 {
		if err := json.Unmarshal(cmd.Envelope.Data, &data); err != nil {
			log.Printf("[task_revision_recorded] event=%s invalid json: %v", cmd.EventType, err)
	...
	log.Printf(
		"[task_revision_recorded] level=info event=%s revision_id=%s task_id=%s tenant_id=%s version_num=%v actor_user_id=%s changed_fields=%s",
		cmd.EventType,
		fieldString(data, "revision_id"),
		fieldString(data, "task_id"),
