...
const cleanupPath = "/api/internal/taskauth/wechat-mp-cleanup/"

// CleanupClient posts to taskAuth internal wechat-mp-cleanup API.
type CleanupClient interface {
	CleanupOnce(ctx context.Context) error
}

// TaskAuthCleanupClient calls taskAuth wechat-mp-cleanup.
...
// CleanupOnce triggers one wechat mp stale-row sweep.
...
func (c *TaskAuthCleanupClient) CleanupOnce(ctx context.Context) error {
	...
	if json.Unmarshal(body, &out) == nil {
		log.Printf("[wechat_mp_cleanup] sweep result: pending_deleted=%v ticket_deleted=%v", out["pending_deleted"], out["ticket_deleted"])
	}
	return nil
}

// CleanupTickInterval reads WECHAT_MP_CLEANUP_TICK_SEC; values <=0 fall back to 24h.
func CleanupTickInterval() time.Duration {
	raw := strings.TrimSpace(os.Getenv("WECHAT_MP_CLEANUP_TICK_SEC"))
	if raw == "" {
		return 24 * time.Hour
	}
...
func timerHealthHandler(domainName string) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		_ = json.NewEncoder(w).Encode(timerHealthState{
			...
			Domain:    domainName,
			Transport: "timer",
			Worker:    "wechat_mp_cleanup",
		})
	}
}

// StartCleanupTicker runs the wechat-mp stale-row cleanup sweep on interval until ctx is cancelled.
func StartCleanupTicker(ctx context.Context, client CleanupClient, interval time.Duration) {
	...
	run := func() {
		if err := CleanupOnce(ctx, client); err != nil {
			log.Printf("[wechat_mp_cleanup] sweep: %v", err)
		}
	}
	run()
	for {
...
// Run starts health HTTP and the wechat-mp cleanup ticker (no Kafka).
func Run(serviceName, host string, port int) {
	...
	log.Printf("[%s] health http://%s/api/health/ worker=wechat_mp_cleanup tick=%s", serviceName, addr, interval)
