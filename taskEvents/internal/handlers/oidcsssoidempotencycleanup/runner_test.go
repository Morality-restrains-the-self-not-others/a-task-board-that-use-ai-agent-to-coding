...
// OPT-20260825-030：taskAuth 实际监听 8003；兜底默认对齐 useraccountdeletionexecute。
...
// CleanupOnce 应命中 cleanup 路径并透传 taskAuth 期望的 X-TaskAuth-Internal-Secret 头。
...
func TestCleanupTickIntervalDefaultAndFallback(t *testing.T) {
	t.Setenv("OIDC_SSO_IDEMPOTENCY_CLEANUP_TICK_SEC", "")
	if got := CleanupTickInterval(); got != 24*3600e9 {
		t.Fatalf("default tick = %v, want 24h", got)
	}
	t.Setenv("OIDC_SSO_IDEMPOTENCY_CLEANUP_TICK_SEC", "0")
	if got := CleanupTickInterval(); got != 24*3600e9 {
		t.Fatalf("fallback tick = %v, want 24h", got)
	}
	t.Setenv("OIDC_SSO_IDEMPOTENCY_CLEANUP_TICK_SEC", "3600")
