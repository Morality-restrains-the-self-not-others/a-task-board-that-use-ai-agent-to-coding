...
func TestApplyAliyunECSNetworkSetsEndpointAndNoProxy(t *testing.T) {
	...
	applyAliyunECSNetwork(cfg, "cn-hongkong")
	if cfg.Endpoint == nil || *cfg.Endpoint != "ecs.cn-hongkong.aliyuncs.com" {
		t.Fatalf("endpoint=%v", cfg.Endpoint)
	}
	if cfg.NoProxy == nil || !strings.Contains(*cfg.NoProxy, "ecs.cn-hongkong.aliyuncs.com") {
