/**
 * daydaymoney.yaml 轻量解析 + 页面 meta 读取（与 task2app daydaymoneyMeta.js 同构）
 */
...
function assertTagConstraints(tags) {
  if (!tags.length) throw new Error('daydaymoney.yaml tags 不能为空')
  if (tags.length > TAGS_MAX_COUNT) {
    throw new Error(`daydaymoney.yaml tags 最多 ${TAGS_MAX_COUNT} 个`)
  }
  for (const tag of tags) {
    if (tag.length > TAG_MAX_LENGTH) {
      throw new Error(`daydaymoney.yaml 标签过长（>${TAG_MAX_LENGTH}）: ${tag}`)
    }
  }
}

function parseDaydaymoneyYaml(text) {
...
function readDaydaymoneyMetaFromDocument(doc) {
  const d = doc || (typeof document !== 'undefined' ? document : null)
  if (!d) return null

  const sidEl = d.querySelector('meta[name="daydaymoney-service-id"]')
  const tagsEl = d.querySelector('meta[name="daydaymoney-tags"]')
...
function readDaydaymoneyMetaEvalExpression() {
  return `(() => {
    const sidEl = document.querySelector('meta[name="daydaymoney-service-id"]');
    const tagsEl = document.querySelector('meta[name="daydaymoney-tags"]');
...
function readDaydaymoneyMetaFromInspectedWindow(inspectedWindow) {
  const iw = inspectedWindow || (typeof chrome !== 'undefined' && chrome.devtools?.inspectedWindow)
  if (!iw?.eval) {
    return Promise.resolve(null)
  }
  return new Promise((resolve) => {
    iw.eval(readDaydaymoneyMetaEvalExpression(), (result, isException) => {
...
const DaydaymoneyMeta = {
  parseDaydaymoneyYaml,
  readDaydaymoneyMetaFromDocument,
  readDaydaymoneyMetaFromInspectedWindow,
...
if (typeof module !== 'undefined' && module.exports) {
  module.exports = DaydaymoneyMeta
}
if (typeof globalThis !== 'undefined') {
  globalThis.DaydaymoneyMeta = DaydaymoneyMeta
}
