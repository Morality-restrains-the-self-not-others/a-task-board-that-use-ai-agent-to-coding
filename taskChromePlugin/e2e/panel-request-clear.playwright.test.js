  ...
  test('清空列表按钮清空 UI 且刷新不再拉回', async ({ page }) => {
    ...
    await page.evaluate(() => {
      for (const fn of window.__reqListeners) {
        fn(window.makeEntryHack('GET', 'https://api.daydaymoney.com/a', 200, '2026-08-05T02:00:00.000Z'));
      }
      for (const fn of window.__reqListeners) {
        fn(window.makeEntryHack('POST', 'https://api.daydaymoney.com/b', 500, '2026-08-05T02:00:01.000Z'));
      }
    });
