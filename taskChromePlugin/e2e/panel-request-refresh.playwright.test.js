...
function loadPlaywrightTest() {
  try {
    ...
    return require(path.resolve(__dirname, '../../DaydaymoneyGrafana/node_modules/@playwright/test'));
  }
}
  ...
  return `
    ...
    const sendMessageImpl = async (msg) => {
      ...
      if (msg.action === 'getAuthStatus') {
        return { success: true, data: { baseUrl: 'https://daydaymoney.com', token: '', loggedIn: false, expired: false, tokenExpiresAt: 0, tokenIssuedAt: 0, remainingSeconds: 0, expiryHint: null } };
      }
...
test.describe('DevTools 面板请求列表刷新链路', () => {
  test('实时推送 + 刷新按钮从 SW 拉取最新请求', async ({ page }) => {
    ...
    await page.evaluate(() => {
      for (const fn of window.__reqListeners) fn(window.makeEntryHack('GET', 'https://api.daydaymoney.com/a', 200, '2026-08-05T01:00:00.000Z'));
      for (const fn of window.__reqListeners) fn(window.makeEntryHack('POST', 'https://api.daydaymoney.com/b', 201, '2026-08-05T01:00:01.000Z'));
    });
    ...
    await panelFrame.evaluate(() => {
      const req = window.makeEntryHack('GET', 'https://api.daydaymoney.com/c', 404, '2026-08-05T01:00:02.000Z');
