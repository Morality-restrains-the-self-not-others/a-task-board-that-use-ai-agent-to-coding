...
describe('buildAuthorizeUrl', () => {
  it('builds authorize URL with all PKCE params and fixed client', () => {
    ...
    const u = new URL(url);
    assert.equal(u.origin, 'https://daydaymoney.com');
  ...
  it('strips trailing slash from baseUrl', () => {
    ...
    assert.ok(url.startsWith('https://daydaymoney.com/api/oidc/authorize'));
  });
});
...
describe('exchangeCodeForToken', () => {
  ...
  it('POSTs JSON body with client credentials + verifier, returns tokens', async () => {
    ...
    assert.equal(seenUrl, 'https://daydaymoney.com/api/oidc/token');
  ...
  it('surfaces readable error with detail when exchange fails', async () => {
    ...
    await assert.rejects(
      () => OAuthPKCE.exchangeCodeForToken({
        baseUrl: 'https://daydaymoney.com', code: 'bad', redirectUri: 'r', codeVerifier: 'v'.repeat(43),
      }),
      /invalid_grant/,
    );
  });
...
describe('refreshAccessToken', () => {
  ...
  it('throws readable error when refresh grant is rejected (rotated/replayed token)', async () => {
    ...
    await assert.rejects(
      () => OAuthPKCE.refreshAccessToken({ baseUrl: 'https://daydaymoney.com', refreshToken: 'stale' }),
      /invalid_grant/,
    );
  });
  ...
  it('throws network error when fetch rejects', async () => {
    ...
    await assert.rejects(
      () => OAuthPKCE.refreshAccessToken({ baseUrl: 'https://daydaymoney.com', refreshToken: 'x' }),
      /刷新网络失败/,
    );
  });
});
...
describe('fetchUserInfo', () => {
  ...
  it('GETs userinfo with Bearer token', async () => {
    ...
    const user = await OAuthPKCE.fetchUserInfo('https://daydaymoney.com', 'eyJ.token');
    assert.equal(seenUrl, 'https://daydaymoney.com/api/oidc/userinfo');
