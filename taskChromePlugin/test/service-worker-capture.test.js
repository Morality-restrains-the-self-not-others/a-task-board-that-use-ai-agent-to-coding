...
function makeRequest({ statusCode = 500, statusLine = 'ERR', url = 'https://api.daydaymoney.com/tasks/', tabId = 7, method = 'GET', withHeaders = true }) {
  return {
    tabId,
    url,
    method,
...
test('集成：10 个已捕获请求在 1s 窗口内合批为 1 次 session 写入，push 瞬间零存储读写', async () => {
  ...
  for (let i = 0; i < 10; i++) {
    await chrome.__webRequestHandlers.onCompleted(makeRequest({ statusCode: 500, url: `https://api.daydaymoney.com/tasks/${i}` }));
  }
  ...
  assert.equal(writes[0][0].url, 'https://api.daydaymoney.com/tasks/0');
  assert.equal(writes[0][9].url, 'https://api.daydaymoney.com/tasks/9');
...
test('集成：clearCapturedErrors 丢弃缓冲 pending，清空后的列表不被下一轮 flush 回写', async () => {
  ...
  for (let i = 0; i < 5; i++) {
    await chrome.__webRequestHandlers.onCompleted(makeRequest({ statusCode: 500, url: `https://api.daydaymoney.com/tasks/c${i}` }));
  }
...
test('F1 缓存：N 个请求只读 1 次 capture 配置（热路径零 storage IPC）', async () => {
  ...
  for (let i = 0; i < 5; i++) {
    await chrome.__webRequestHandlers.onErrorOccurred({
      tabId: 7,
      url: 'https://api.daydaymoney.com/canceled/',
