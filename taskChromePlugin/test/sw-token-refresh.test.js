...
describe('SW OAuth access token 静默续期', () => {
  ...
  it('getAuthStatus：过期 + 持 refresh token → 静默刷新并返回未过期状态', async () => {
    ...
    assert.equal(resp.data.baseUrl, 'https://daydaymoney.com');
    ...
    assert.equal(call.url, 'https://daydaymoney.com/api/oidc/token');
  ...
  it('getAuthStatus：未过期 → 不发起刷新请求', async () => {
    const { send, refreshCalls } = loadSW({
      storageSeed: {
        baseUrl: 'https://daydaymoney.com', token: 'tok-ok',
  ...
  it('getAuthStatus：过期但无 refresh token（旧版登录态）→ 不刷新，保持过期（向后兼容）', async () => {
    const { send, refreshCalls } = loadSW({
      storageSeed: {
        baseUrl: 'https://daydaymoney.com', token: 'tok-expired',
  ...
  it('oauthRefresh 无 refresh token → refreshed:false 且不报错', async () => {
    const { send, refreshCalls } = loadSW({
      storageSeed: { baseUrl: 'https://daydaymoney.com', token: 'tok-ok', tokenExpiresAt: FUTURE_AT, authExpiryMigratedV161: true },
      fetchImpl: async () => { throw new Error('should not be called'); },
    });
    const resp = await send({ action: 'oauthRefresh' });
    assert.equal(resp.success, true);
