...
describe('buildCapturedEntry 字段分层', () => {
  it('2xx 成功请求只存最小字段，不存任何 headers', () => {
    const entry = buildCapturedEntry({
      url: 'https://daydaymoney.com/api/tasks/?page=2',
      method: 'GET',
      statusCode: 200,
      statusLine: 'OK',
      type: 'xmlhttprequest',
    ...
    assert.equal(entry.url, 'https://daydaymoney.com/api/tasks/?page=2');
  ...
  it('3xx 成功类请求同样不存 headers', () => {
    const entry = buildCapturedEntry({
      url: 'https://daydaymoney.com/api/x',
      method: 'GET',
      statusCode: 302,
      statusLine: 'Found',
      type: 'main_frame',
  ...
  it('4xx 错误请求保留裁剪后的完整头', () => {
    const entry = buildCapturedEntry({
      url: 'https://daydaymoney.com/api/tasks/',
      method: 'POST',
      statusCode: 404,
  ...
  it('canceled 请求保留裁剪后的完整头（诊断价值）', () => {
    const entry = buildCapturedEntry({
      url: 'https://daydaymoney.com/api/stream',
      method: 'GET',
      statusCode: 0,
      statusLine: 'Canceled',
      type: 'xmlhttprequest',
