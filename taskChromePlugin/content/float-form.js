...
async function loadWorkspaces() {
  ...
  try {
    ...
    await applyDaydaymoneyMetaAfterWorkspacesLoaded();
  } catch (e) {
...
function checkAidevMatchingProjects(wsId) {
  if (!pendingAidevMatches?.length || typeof DaydaymoneyMeta === 'undefined') return;
  const pids = DaydaymoneyMeta.projectIdsForWorkspace(pendingAidevMatches, wsId);
...
async function applyDaydaymoneyMetaAfterWorkspacesLoaded() {
  pendingAidevMatches = null;
  if (typeof DaydaymoneyMeta === 'undefined') {
    setAidevStatus('', false);
    return;
  }

  const meta = DaydaymoneyMeta.readDaydaymoneyMetaFromDocument(document);
  if (!meta?.service_id) {
    setAidevStatus('', false);
    return;
  }
  ...
  try {
    const resp = await swApi('resolveDaydaymoneyMeta', { serviceId: meta.service_id });
    const matches = Array.isArray(resp?.matches) ? resp.matches : [];
    setAidevStatus(DaydaymoneyMeta.formatAidevResolveStatus(matches));
    if (!matches.length) return;

    pendingAidevMatches = matches;
    const wsIds = DaydaymoneyMeta.uniqueWorkspaceIdsFromMatches(matches);
    if (wsIds.length === 1) {
    ...
    console.warn('[taskChromePlugin] daydaymoney resolve 失败:', e.message);
