...
(function () {
  ...
  P.loadWorkspaces = async function (selectId) {
    ...
    try {
      ...
      const aidevWsId = await P.applyDaydaymoneyMetaAfterWorkspacesLoaded(selectId);
  ...
  P.setPanelAidevStatus = function (text, visible = true) {
    const el = P.$('#panel-daydaymoney-status');
    if (!el) return;
    if (!visible || !text) {
      el.hidden = true;
      el.textContent = '';
  ...
  P.applyDaydaymoneyMetaAfterWorkspacesLoaded = async function (selectId) {
    ...
    const meta = await DaydaymoneyMeta.readDaydaymoneyMetaFromInspectedWindow();
    if (!meta?.service_id) {
      P.setPanelAidevStatus('', false);
      return null;
    }
    ...
    try {
      const resp = await P.swApi('resolveDaydaymoneyMeta', { serviceId: meta.service_id });
      const matches = Array.isArray(resp?.matches) ? resp.matches : [];
      P.setPanelAidevStatus(DaydaymoneyMeta.formatAidevResolveStatus(matches));
      ...
      console.warn('[taskChromePlugin] panel daydaymoney resolve:', e.message);
