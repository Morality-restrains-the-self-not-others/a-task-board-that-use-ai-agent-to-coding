...
CLEANUP_SCRIPT = ROOT / "scripts" / "lib" / "test_resource_cleanup.sh"
...
def run_test(repo: Path, kind: str, target: str) -> tuple[bool, str]:
    ...
    try:
        ...
        # 测例挂死（如 SSE server 测例启动后不退出）→ 记为 [TIMEOUT] 失败继续巡检，
        # 不抛出异常炸掉整轮（曾导致 01:20 轮在 Round 1 整体崩溃）
        return False, f"[TIMEOUT {SINGLE_RUN_TIMEOUT_SEC}s] {target} 挂死被杀"
...
def session_hub_has_interactive() -> bool:
    """OPT-20260823-020: Session Hub 有 kind=interactive 的活跃会话时返回 True。

    交互会话的 pre-commit 会打同一 docker-mysql；sweep 此时跳过打 MySQL 的 Go 仓，
    避免并发 DDL/fsync 风暴（LRN-20260823-001）。二进制/机制不可用时返回 False（不阻塞巡检）。
    """
    bin_agent = ROOT / "claude-agent" / "bin" / "claude-agent"
    if not bin_agent.is_file():
        return False
    ...
    try:
        ...
        return False
    return any((s.get("kind") or "") == "interactive" for s in sessions)
...
def ensure_api_key_env() -> None:
    """cron 环境不加载 ~/.bashrc（其交互守卫使非交互 shell 提前 return），
    ANTHROPIC_API_KEY 缺失时从 ~/.bashrc 的 export 行补齐。
    否则 claude-agent 子进程拿到空 key → 403 → 修复闭环整体失败。"""
    if os.environ.get("ANTHROPIC_API_KEY"):
        return
    rc = Path.home() / ".bashrc"
    try:
        for ln in rc.read_text(encoding="utf-8").splitlines():
            ln = ln.strip()
            if ln.startswith("export ANTHROPIC_API_KEY="):
...
def main() -> int:
    ...
    while rounds_run < args.max_rounds:
        ...
        deadline_reached = False
        results: list[dict] = []
        # OPT-20260823-020: 交互会话活跃时跳过打 MySQL 的 Go 仓（每轮查一次 Session Hub）
        has_interactive = session_hub_has_interactive()
        ...
        for name, repo in repos:
            ...
            # OPT-20260823-020: 交互会话活跃时跳过打 MySQL 的 Go 仓（其 pre-commit 会并发建库）
            if has_interactive and "go" in rtype:
                results.append({"repo": name, "result": "SKIP",
                                "note": "interactive session active; skip MySQL-hitting Go repo (OPT-20260823-020)"})
                continue
            ...
            if timeouts:
                # 含挂死超时：修复 agent 重跑复现会同样挂死（白烧 25min），跳过修复闭环，
                # 结果记为 TIMEOUT 与真实失败区分（真实失败会在后续轮次被重新抽中）
                results.append({"repo": name, "result": "TIMEOUT",
                                "note": f"timeout={timeouts} other_failures={[t for t in failures if t not in timeouts] or 'none'}",
    ...
    # 测试残留扫描（约束 44 / OPT-20260810-004）：只读扫描，残留非零（退出码 2）
    # 即输出告警并记录残留清单到报告；--fix 由人工按告警手动执行，夜间不自动清理
    # （runAll 托管服务窗口内全量运行，自动清理存在误杀风险）。
    md.append("### 测试残留扫描（约束 44）")
    md.append("")
    try:
        trc = run(["bash", str(CLEANUP_SCRIPT)], timeout=300)
        if trc.returncode == 2:
            md.append("**⚠️ 发现测试残留** — 清理命令: `bash scripts/lib/test_resource_cleanup.sh --fix`")
            md.append("")
            md.append("```")
            md.append(redact((trc.stdout or trc.stderr or "")[:4000]))
            md.append("```")
            print(f"[sweep] test resource scan: RESIDUAL FOUND (exit=2)")
        elif trc.returncode == 0:
            md.append("✅ 无测试资源残留")
            md.append("")
            print(f"[sweep] test resource scan: clean")
        else:
            md.append(f"⚠️ 扫描异常（exit={trc.returncode}）: " + redact((trc.stdout or trc.stderr or "")[:500]))
            md.append("")
            print(f"[sweep] test resource scan: abnormal exit={trc.returncode}")
    except subprocess.TimeoutExpired:
        md.append("⚠️ 扫描超时（>300s），跳过残留记录")
        md.append("")
        print("[sweep] test resource scan: TIMEOUT")
