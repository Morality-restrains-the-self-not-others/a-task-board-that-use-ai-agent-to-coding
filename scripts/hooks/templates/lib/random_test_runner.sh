...
rt_go_run_dir() {
    ...
    if [ "$dir" = "." ]; then
        ...
        output="$(rt_mysql_test_lock_run "go test -count=1 ." 2>&1)" || {
        ...
        output="$(rt_mysql_test_lock_run "go test -count=1 ${dir}/..." 2>&1)" || {
            if echo "$output" | grep -qE 'cannot find main module|does not contain main module'; then
                ...
                output="$(rt_mysql_test_lock_run "(cd \"${dir#./}\" && go test -count=1 ./...)" 2>&1)" || {
                    if echo "$output" | grep -q 'matched no packages'; then
                        echo "Skipping ${dir}: no packages to test (build tags)"
                        return 0
                    fi
