...
部署（v13，仓库根）——渲染至各子仓 `.githooks/`（入库真源）+ `core.hooksPath` 激活：

```bash
bash scripts/deploy_repo_random_precommit.sh          # 全部子仓
bash scripts/deploy_repo_random_precommit.sh taskAuth # 指定子仓
bash runAll/scripts/install-hooks-all.sh --check      # 校验激活状态
```

> v13（git-hooks-version-control）：`.git/hooks/` 复制模式已退役；钩子版本戳见各仓 `.githooks/HOOK_VERSION`。
