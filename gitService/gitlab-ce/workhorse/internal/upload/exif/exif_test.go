...
// skipIfNoExifTool skips the test when the exiftool binary is not available
// in the environment. These tests exercise the real exiftool subprocess, so
// they are meaningless without it.
func skipIfNoExifTool(t *testing.T) {
	t.Helper()
	if _, err := exec.LookPath("exiftool"); err != nil {
		t.Skip("exiftool not found in PATH, skipping test that requires it")
	}
}
