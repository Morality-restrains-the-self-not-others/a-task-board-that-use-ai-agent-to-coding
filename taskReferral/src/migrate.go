...
func runDataMigrateFromDir(dsn string, repoRoot string) error {
	migDir := filepath.Join(repoRoot, "dataMigrate", "taskReferral")
	...
	// Backfill from legacy taskreferral_schema_migrations into data_migrate_log
	var count int
	if err := conn.QueryRow(`SELECT COUNT(*) FROM data_migrate_log`).Scan(&count); err == nil && count == 0 {
		rows, err := conn.Query(`SELECT name, applied_at FROM taskreferral_schema_migrations ORDER BY name`)
		if err == nil {
			defer rows.Close()
			for rows.Next() {
	...
	for _, name := range files {
		...
		if _, err := conn.Exec(mysqlmeta.StripMySQLClientMeta(string(raw))); err != nil {
