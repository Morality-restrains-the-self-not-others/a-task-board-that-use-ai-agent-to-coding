...
4. **公网 SPA 已构建** — if the change set includes production SPA sources under `taskFE/app/src`, confirm `npm run build` (Vite build → `taskFE/app/dist/`) ran and preview serves it with 200; else record `n/a` with reason in the summary（Django `collectstatic` 已退役）
...
### Project gates
- [ ] 公网 SPA 已构建 — done | n/a (<reason>) | missing
