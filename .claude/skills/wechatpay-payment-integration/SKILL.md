...
## 本仓库落地（实现代码时优先于下方官方交互规范）

在本 monorepo **编写或修改微信支付调用**时：

1. **必须**使用仓内 [`sdk/wechatpay-go`](../../../sdk/wechatpay-go)（Go module `github.com/wechatpay-apiv3/wechatpay-go`）。
2. 依赖该 module 的服务 `go.mod` **必须** `replace` 到 `sdk/wechatpay-go`。
3. 优先调用 `services/<产品>`（Native / 退款 / 分账等）；回调走 `core/notify`。无生成接口时才用 `core.Client`。
4. **禁止**社区 SDK（如 go-pay）、APIv2 XML、手写 `WECHATPAY2-SHA256-RSA2048`、裸 HTTP 直打微信网关。
5. 权威约束：`.ai/01_project_constraints/59_wechatpay_go_sdk.md`（ADR-0032）。产品选型、官方文档检索与排障仍走本 Skill 知识库。
6. 租户资源订单 Native 预下单：**一律**设置 `native.PrepayRequest.SettleInfo.ProfitSharing=true`（ADR-0040），无论是否有推荐人；禁止手搓 `settle_info` JSON。佣金台账与打款仍按支付时刻推荐资格（ADR-0033 剩余条款）。
...
2. **官方语言（curl / Java / Go）**：按[知识库使用指南](./assets/wechatpay-docs-guide.md)定位到该产品 `API列表/` 下的接口文档，读取对应语言的请求示例文件输出。**本仓库 Go 实现必须改写为 `sdk/wechatpay-go` 的 `services/<产品>` 调用**（import `github.com/wechatpay-apiv3/wechatpay-go/...`），不得把文档里的裸 HTTP 示例直接贴进业务服务。前端调起 / 回调类接口无后端请求示例时，直接给出该接口文档内容。
