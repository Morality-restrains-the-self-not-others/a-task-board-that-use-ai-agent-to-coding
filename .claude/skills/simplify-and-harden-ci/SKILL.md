...
7) Project gate: if the PR touches `taskFE/app/src` SPA production sources, emit an advisory (or critical when policy enabled) when there is no evidence the author ran `npm run build` (Vite build → `dist/`) — checklist item 「公网 SPA 已构建」. Put result in `project_gates.spa_build` (`done` | `n/a` | `missing`). Django `collectstatic` 已退役，非有效构建证据。
8) Emit structured YAML under key `simplify_and_harden`, including:
   - simplify findings
   - harden findings (critical/advisory split)
   - `project_gates.spa_build`
   - summary counts
   - `review_followup_required`
   - learning loop candidates for self-improvement ingestion
9) If blocking policy is enabled and matching findings exist, mark the run as failed.
```
