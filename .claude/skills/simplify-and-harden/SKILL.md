...
### Chesterton's Fence（切斯特顿之栏）

Before changing or removing anything, understand why it exists. If you see a fence across a road and don't understand why, don't tear it down. Check `git blame` — understand the original context, then decide if the reason still applies.

### The Rule of 500

If a refactoring would touch more than 500 lines, invest in automation (codemods, sed scripts, AST transforms) rather than manual edits. Manual edits at that scale are error-prone and exhausting to review.
...
## Pass 4: Project gates（本仓库强制勾选）

**Objective:** Catch deploy-time failures that simplify/harden/document will not see in source alone.

After Pass 3, if the task modified files under `taskFE/app/src/` that affect the production SPA bundle (see `taskFE/app/ai.md`), complete this checklist before signaling done:

### Review Checklist

- [ ] **公网 SPA 已构建** — 已执行 `npm run build`（Vite build，写出 `taskFE/app/dist/`），且 `vite preview`（或部署态静态服务）对 dist 产物返回 **200**。Django 已退役，`collectstatic` 为历史操作，禁止在 build 链上挂回（否则 `test_collectstatic_after_vite.sh` 回归失败）。
- [ ] **不适用已注明** — 若本次仅改测例/文档/companion、或用户明确不要求公网即时生效，在输出摘要中写 `spa_build: n/a` 与理由；不得默默跳过。

关联：`taskFE/app/ai.md`、`.ai/09_failure_experience/02_runtime_errors/04_public_spa_static_js_404_after_vite_build.md`、`.claude/skills/10-ship/SKILL.md`。
...
simplify_and_harden:
  ...
  project_gates:
    spa_build: "done"  # "done" | "n/a" | "missing"（历史字段名 spa_collectstatic，已随 Django 退役改名）
    spa_build_note: "ran npm run build; dist/preview 200"
...
5. **Three passes** -- simplify, harden, document (in that order), then **project gates** (SPA build when applicable)
6. **Structured output** -- summary of applied, approved, rejected, and flagged items (include `project_gates.spa_build`)
...
5. Project gates: If you changed `taskFE/app/src` SPA sources, check off
   「公网 SPA 已构建」— run `npm run build` (Vite build → `dist/`) and confirm
   `vite preview` serves it with 200; otherwise mark n/a with reason.
6. Output a summary of what you changed, what you flagged, and 
   what you left alone (include `project_gates.spa_build`).
```
