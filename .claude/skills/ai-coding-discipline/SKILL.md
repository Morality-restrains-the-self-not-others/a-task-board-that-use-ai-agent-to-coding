...
description: >
  ...
  中文：编写/修改代码、测试、修 bug、代码审查时加载；禁止静默回退、业务层吞错、弱断言、硬编码凑测试、修 bug 时与调试日志同删等反模式。
license: MIT
metadata:
  source: https://github.com/luoling8192/ai-coding-principles
  upstream_path: ai-coding-discipline/SKILL.md
---
...
**Project overlay (this monorepo):** If the bug report / UI snapshot includes **`data-traceId`** (or an extractable traceId), **query Loki/Grafana first** and reconstruct the full cross-service path before editing code. Do not guess from the error string alone. See `.claude/skills/1-brainstorming-design-docs/references/traceid-log-first-diagnosis.md`.
...
| Intent → event | For mutating/cross-boundary intents: is the mapped domain event published to MQ (or an explicit no-event exception documented)? |

If Fallbacks/Error/Test/Debug answers are "yes", revise. If Intent→event answer is "no", revise before proceeding.

---

## Rule: Business Intent Must Publish MQ Event

**When a business intent is accepted, publish the corresponding business/domain event to the message queue.**

```python
# FORBIDDEN — intent succeeds, side effects done synchronously, no event
def register_user(cmd):
    user = User.create(cmd)
    repo.save(user)
    email_client.send_welcome(user.email)  # sync cross-boundary
    return user

# CORRECT — intent → event → MQ; consumers handle side effects
def register_user(cmd, event_bus: EventBusPort):
    user = User.create(cmd)  # records UserRegistered
    repo.save(user)
    event_bus.publish(user.pull_domain_events())
    return user
```

**Exceptions (must be written in `docs/intents/*.intent.md`):** pure queries/reads; frontend-only with no server state change.

**Canonical rule:** `.ai/08_prompt_management/01_intent_driven_development.md`
