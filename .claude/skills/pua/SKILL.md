...
**铁律四（本仓库）：有 data-traceId 先查日志**。错误 UI / 快照 / 用户粘贴中出现 **`data-traceId`**（或可提取的 traceId）时，**必须优先**用 Loki/Grafana 检索该 ID 的全量日志，按时间线重建整条错误发生路径，再猜根因或改代码。未查日志就凭文案空转 = 3.25。短规范：`.claude/skills/1-brainstorming-design-docs/references/traceid-log-first-diagnosis.md`；完整步骤见 brainstorming「TraceId 驱动的 Grafana 日志分析」。
...
| 行为 | 被动（3.25） | 主动（3.75） |
|------|------------|------------|
| 遇到报错 | 只看报错信息本身；有 `data-traceId` 却不查 Loki | 有 `data-traceId` → **先**拉全链路日志重建路径；再查上下文 50 行 + 同类问题 + 关联错误 |
...
1. **逐字读失败信号**。错误信息、拒绝原因、空结果、用户的不满意——不是扫一眼，是逐字读。90% 的答案你直接忽略了。**若含 `data-traceId` / traceId：立刻记下 ID，下一步必须先查日志（铁律四），不要跳到猜源码。**

2. **主动搜索**。不要靠记忆和猜测——让工具告诉你答案：
 - 代码场景且有 traceId → **优先** Loki/Grafana 按 `trace_id` 全 job 检索，重建跨服务路径
 - 代码场景无 traceId → 搜索完整报错信息
 - 研究场景 → 搜索多个关键词角度
 - API/工具场景 → 搜索官方文档 + Issues

3. **读原始材料**。不是读摘要或你的记忆，是读原始来源：
 - 有 traceId → 日志时间线（各服务 ERROR/WARN + 首错前后 INFO）是第一手材料
 - 代码场景 → 出错文件上下文 50 行（在日志路径指向之后）
 - API 场景 → 官方文档原文
 - 研究场景 → 原始来源，不是二手引用
...
- [ ] **data-traceId → 日志优先**：若有 `data-traceId`/traceId，是否已查 Loki 并写出跨服务时间线？（未查 = 未完成本项）
- [ ] **主动搜索**：用工具搜索过核心问题了吗？（代码：报错原文 / 研究：多角度关键词 / API：官方文档）
- [ ] **读原始材料**：读过失败位置的原始上下文了吗？（有 trace：日志路径 / 代码：源码50行 / API：文档原文 / 数据：原始文件）
...
| 🔍 **没搜索就猜** | 凭记忆下结论、假设 API 行为、不查文档声称"不支持"；**有 `data-traceId` 却不查 Loki** | ⚫ 百度味 | 🟡 字节味 | 🟠 阿里味 | 🔴 华为味 |
...
- `.claude/skills/1-brainstorming-design-docs/references/traceid-log-first-diagnosis.md` — 有 `data-traceId` 时先日志重建路径（铁律四）
