...
> 被排障类技能引用的短规范。完整 Loki 查询、空结果诊断、Grafana 链接见父目录 [`SKILL.md`](../SKILL.md)「TraceId 日志优先 — 横切硬门禁」及 [`1-brainstorming-design-docs/SKILL.md`](../../1-brainstorming-design-docs/SKILL.md)「TraceId 驱动的 Grafana 日志分析」。
...
| 入口 | 要求 |
|------|------|
| `/goal` — 直接执行模式 | Step 0 强制检测；有 traceId 先查日志再进入 Planning |
| `/goal` — 流水线模式 | 横切硬门禁，优先级高于 Task Routing |
